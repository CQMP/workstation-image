...
\subsection*{Pending: database backup}

The Tuwunel RocksDB database at \texttt{/var/lib/tuwunel} on the Oracle VM is
not yet backed up.  A nightly rsync to the Synology over the SSH tunnel should
be added.  Media is already safe (stored on the Synology via MinIO); only the
RocksDB database (user accounts, room state, message indices) needs this backup.
